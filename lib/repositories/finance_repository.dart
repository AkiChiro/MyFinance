...
class FinanceRepository {
  ...
  Future<Map<String, int>> categoryTotals(String txType, DateTime month) async {
    ...
    for (final t in all) {
      ...
      final cat = t.category ?? Categories.fallbackFor(txType);
      result[cat] = (result[cat] ?? 0) + t.amount;
    }
    return result;
  }

  Future<int> monthTotal(String txType, DateTime month) async {
    final map = await categoryTotals(txType, month);
    return map.values.fold<int>(0, (a, b) => a + b);
  }

  Future<int> yearTotal(String txType, int year) async {
    final all = await db.nativeTxns();
    var total = 0;
    for (final t in all) {
      if (t.type != txType) continue;
